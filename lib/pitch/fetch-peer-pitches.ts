...
function competenciesFromTitle(title: string) {
  const match = PITCH_SCENARIOS.find((scenario) =>
    title.toLowerCase().includes(scenario.label.toLowerCase().slice(0, 8)),
  );
  return match?.competencies ?? [];
}

export async function fetchPeerPitches(
  ...
  if (profile?.manager_id) {
    ...
    teammateIds = (teammates ?? []).map((row) => row.id).filter((id) => id !== userId);
  }
  ...
  const { data: peerReviews } = pitchIds.length
    ? await supabase
        ...
        .select("pitch_id, clarity_score, storyline_score, differentiation_score, endorsed")
        .in("pitch_id", pitchIds)
    : { data: [] };

  const reviewsByPitch = new Map<
    string,
    Array<{
      pitch_id: string;
  ...
  const { data: profiles } = userIds.length
    ...
    : { data: [] };
  const nameById = new Map((profiles ?? []).map((entry) => [entry.id, entry.full_name]));

  const pitches = (rows ?? []).map((row) => {
    const reviews = reviewsByPitch.get(row.id) ?? [];
    const peerScores = reviews.map((review) => (review.clarity_score + review.storyline_score + review.differentiation_score) / 3);
    const peerAvg = peerScores.length > 0 ? peerScores.reduce((a, b) => a + b, 0) / peerScores.length : null;
    const endorsementCount = reviews.filter((review) => review.endorsed).length;
