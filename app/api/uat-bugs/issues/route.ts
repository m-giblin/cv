...
import { forgeEnvironmentForTenant, issueMatchesTenant } from "@/lib/forge/tenant-scope";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      ...result,
      data: result.data.filter((issue) => issueMatchesTenant(issue.environment, tenantId)),
    });
  } catch (error) {
...
export async function POST(request: Request) {
  ...
  if (contentType.includes("multipart/form-data")) {
    return handleMultipartCreate(request, session.user, tenantId);
  }
  ...
  try {
    const issue = await createForgeIssueFromPayload(parsed.data, session.user.email ?? undefined, tenantId);
    return NextResponse.json(
...
async function handleMultipartCreate(
  ...
  try {
    const issue = await createForgeIssueFromPayload(parsed.data, user.email, tenantId);
...
async function createForgeIssueFromPayload(
  ...
  const issue = await createForgeIssue({
    ...
    environment: forgeEnvironmentForTenant(tenantId),
  });
