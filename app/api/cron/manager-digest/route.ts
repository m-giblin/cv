...
export async function GET(request: Request) {
  ...
  for (const manager of managers ?? []) {
    const { data: reports } = await admin.from("profiles").select("*").eq("manager_id", manager.id);
    ...
    const [
      ...
      admin.from("challenge_submissions").select("*").in("user_id", reportIds),
      admin.from("coaching_cards").select("*").in("user_id", reportIds),
      admin
        .from("activity_logs")
        .select("*")
        .in("user_id", reportIds)
        .order("created_at", { ascending: false }),
      admin.from("plan_assignments").select("*").in("user_id", reportIds),
    ]);
