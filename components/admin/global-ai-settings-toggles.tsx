...
export function GlobalAiSettingsToggles() {
  ...
  return (
    <div className="rounded-xl border border-[#e2eaf5] bg-white p-[18px_22px]">
      <p className="mb-[14px] text-[12.5px] font-bold text-[#0a1628]">Global AI settings</p>
      <div className="space-y-[14px]">
        {settings.map((item) => (
          <div className="flex items-center justify-between border-b border-[#f1f5f9] py-[10px]" key={item.id}>
            <div className="pr-4">
              <p className="text-[12px] font-semibold text-[#1e293b]">{item.label}</p>
              <p className="mt-[1px] text-[11px] text-[#94a3b8]">{item.description}</p>
            </div>
            <Toggle
