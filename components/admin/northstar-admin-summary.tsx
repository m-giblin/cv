...
type PendingReviewBreakdownView = PendingReviewBreakdown;
...
function activityTypeBadge(eventType: ActivityLog["eventType"]) {
 ...
 manager_feedback_received: { type: "Admin", typeBg: "#ECEAE6", typeColor: "#6B6860" },
 plan_assigned: { type: "Plan", typeBg: "#e8f2fc", typeColor: "#0057a8" },
 deal_prep_completed: { type: "AI", typeBg: "#ede9fe", typeColor: "#5b21b6" },
 pitch_submitted: { type: "Pitch", typeBg: "#fce7f3", typeColor: "#be185d" },
 flight_check_completed: { type: "Flight", typeBg: "#e0f2fe", typeColor: "#0369a1" },
 };
...
export function NorthstarAdminSummary({
 ...
 pendingReviewBreakdown?: PendingReviewBreakdownView;
 ...
 const heroPendingReviews = pendingReviewBreakdown ? totalPending : pendingReviews;
 ...
 { value: heroPendingReviews, label: "Pending reviews", color: "text-[#f59e0b]" },
 ...
 <p className="mb-[12px] text-[12.5px] font-bold text-[#0D0E12]">Program health</p>
 ...
 <span className="text-[11px] text-[#6B6860]">{bar.label}</span>
 ...
 <p className="mt-[10px] text-[10.5px] text-[#A09D98]">
 ...
 <p className="text-[12.5px] font-bold text-[#0D0E12]">AI usage · this month</p>
 ...
 <div
 className="flex items-center justify-between border border-[#ECEAE6] bg-[#F9F8F6] px-[10px] py-[8px]"
 key={item.label}
 >
 <div className="flex items-center gap-[8px]">
 <span className="h-[6px] w-[6px] flex-shrink-0 rounded-full" style={{ background: item.dot }} />
 <span className="text-[11.5px] text-[#3D3C38]">{item.label}</span>
 </div>
 <div className="text-right">
 <p className="font-display text-[14px] font-extrabold leading-none text-[#0D0E12]">{item.count}</p>
 <p className="text-[9px] text-[#A09D98]">{item.sub}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <div className="border border-[#E2DFD9] bg-white p-[16px_18px]">
 <div className="mb-[12px] flex items-center justify-between">
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Pending reviews</p>
 ...
 <span className="h-[8px] w-[8px] flex-shrink-0 " style={{ background: item.color }} />
 <span className="text-[11.5px] text-[#3D3C38]">{item.label}</span>
 </div>
 <span className="text-[12.5px] font-bold text-[#0D0E12]">{item.count}</span>
 </div>
 ))}
 </div>
 ...
 <div className="flex items-center justify-between border-b border-[#ECEAE6] p-[13px_18px]">
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Recent platform activity</p>
 <button
 ...
 <p className="px-[18px] py-6 text-sm text-[#A09D98]">No activity yet.</p>
 ) : (
 ...
 <span className="font-semibold text-[#3D3C38]">{actor?.fullName ?? "User"}</span>{" "}
 <span className="text-[#6B6860]">{item.title}</span>
 </p>
 <span
 className="flex-shrink-0 font-mono text-[8px] uppercase tracking-[0.08em] px-[7px] py-[2px] text-[9.5px] font-bold"
 style={{ background: badge.typeBg, color: badge.typeColor }}
 >
 {badge.type}
 </span>
 <span className="flex-shrink-0 text-[10.5px] text-[#A09D98]">{formatRelativeTime(item.createdAt)}</span>
 </div>
 );
 })
 )}
