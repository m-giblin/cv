...
export function PracticeThisWeekCard({ items }: { items: PracticeWeekItem[] }) {
  ...
  return (
    <div className="mb-5 overflow-hidden rounded-xl border border-[#0071ce]/25 bg-gradient-to-br from-[#e8f2fc] to-white shadow-[0_1px_4px_rgba(0,20,58,0.05)]">
        ...
        {items.map((item, index) => (
          <Link
            className={`flex items-center gap-3 border-b border-slate-100 py-3 last:border-b-0 ${
              index === 0 ? "rounded-lg bg-white/80 px-2 -mx-2" : ""
            }`}
            href={item.href}
            key={item.id}
          >
