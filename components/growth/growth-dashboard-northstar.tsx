...
function stageBadge(level: SeLevel, current: SeLevel) {
 ...
 if (stageIndex === currentIndex + 1) return { label: "Next", className: "bg-[#ECEAE6] text-[#6B6860]" };
 if (stageIndex < currentIndex) return { label: "Achieved", className: "bg-[#dcfce7] text-[#15803d]" };
 return { label: "Future", className: "bg-[#ECEAE6] text-[#A09D98]" };
}
...
export function GrowthDashboardNorthstar({ data }: { data: DashboardData }) {
 ...
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Plan progress</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">
 {plan?.progress ?? 0}%
 </p>
 <p className="mt-1.5 text-[10px] text-[#A09D98]">{plan?.name ?? "No active plan"}</p>
 </div>
 ...
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Certifications</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">
 {approvedCerts.length}
 <span className="text-base font-medium text-[#A09D98]"> / {ALL_CERT_ORDER.length}</span>
 </p>
 {nextCertLabel ? (
 ...
 <p className="mt-2 text-[10px] text-[#A09D98]">All gates complete</p>
 )}
 </div>
 <div className={cn(SP_CARD, "border-l-[3px] p-[14px_16px]")} style={{ borderLeftColor: "#f59e0b" }}>
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Practice cadence</p>
 ...
 <div>
 <p className="text-[13.5px] font-bold text-[#0D0E12]">Career readiness map</p>
 ...
 <span className="text-[#6B6860]">Readiness</span>
 ...
 isCurrent ? "text-[#0071ce]" : isPast ? "text-[#6B6860]" : "text-[#A09D98]",
 )}
 >
 {stage.title}
 </span>
 ...
 isCurrent ? "text-[#0D0E12]" : "text-[#3D3C38]",
 )}
 >
 {stage.title}
 </p>
 ...
 isCurrent ? "bg-[#0071ce]" : isPast ? "bg-[#10b981]" : "bg-[#B0ADA8]",
 )}
 />
 <span
 className={cn(
 "text-[11px]",
 isCurrent ? "text-[#374151]" : "text-[#A09D98]",
 )}
 >
 {req}
 </span>
 ...
 isCurrent ? "text-[#0071ce]" : "text-[#A09D98]",
 )}
 >
 ...
 : "border-dashed border-[#A09D98]",
 )}
 />
 <span className={cn("text-[11px]", isCurrent ? "text-[#6B6860]" : "text-[#A09D98]")}>
 ...
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Recommended next practice</p>
 ...
 <div className={cn(SP_CARD, "p-[16px_18px]")}>
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Competency gaps to close</p>
 <p className="mb-2.5 text-[11px] text-[#6B6860]">Based on your last coaching cards</p>
 <div className="flex flex-col gap-2">
 {competencyGaps.length === 0 ? (
 <p className="text-[11px] text-[#A09D98]">No major gaps detected — keep practicing.</p>
 ) : (
 competencyGaps.map((gap) => (
 <div
 className="flex items-center gap-2.5 border border-[rgba(245,158,11,0.2)] bg-[#fef9ec] p-[9px_11px]"
 key={gap.competencyId}
 >
 <div className="flex h-7 w-7 shrink-0 items-center justify-center bg-[#fef3c7]">
