...
function commentPanelClass(status: string) {
  ...
  if (status === "reviewed" || status === "approved") {
    return "border-[#f1f5f9] bg-[#f8fafd]";
  }
  ...
  return (
    <div className="animate-[fadeUp_0.2s_ease-out] space-y-5">
      ...
      {pendingRows.length > 0 ? (
        <section>
          <h2 className="mb-3 flex items-center gap-2 font-display text-[13.5px] font-bold text-[#0a1628]">
            Pending review
                  ...
                  <div className="flex items-start gap-3 p-[14px_18px]">
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="mb-1 flex flex-wrap items-center gap-1.5">
                        ...
                        {row.grade !== null ? (
                          <span className="font-display text-[13px] font-extrabold text-[#0a1628]">
                            {row.grade}/5
                          </span>
                        ) : null}
                      </div>
                      <p className="text-[13px] font-bold text-[#0a1628]">{row.title}</p>
      ...
      <section>
        <h2 className="mb-3 font-display text-[13.5px] font-bold text-[#0a1628]">Feedback history</h2>
        {historyRows.length === 0 ? (
          ...
          <div className={cn(CARD, "overflow-hidden")}>
            <div
              className="grid gap-0 border-b border-[#f1f5f9] bg-[#f8fafd] px-[18px] py-[9px]"
              style={{ gridTemplateColumns: "1fr 100px 100px 60px 100px" }}
            >
              {["Item", "Type", "Status", "Grade", "Reviewed"].map((header) => (
                <span
                  className="text-[9.5px] font-bold uppercase tracking-[0.07em] text-[#94a3b8]"
                  key={header}
                >
                  {header}
                </span>
            ...
            {historyRows.map((row) => (
              <div
                ...
                style={{ gridTemplateColumns: "1fr 100px 100px 60px 100px" }}
              >
                <p className="truncate pr-2 text-[12px] font-semibold text-[#1e293b]">{row.title}</p>
