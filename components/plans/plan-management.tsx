...
function templateVisuals(template: PlanTemplate, index: number) {
 ...
 return { accentGradient, badge: "Draft", badgeBg: "#ECEAE6", badgeColor: "#6B6860" };
}
...
function assignmentStatusStyle(status: UserPlan["status"]) {
 ...
 return { label: "Not started", statBg: "#ECEAE6", statColor: "#6B6860" };
 }
...
export function PlanManagementPanel({
 ...
 return (
 <div
 className={`overflow-hidden border bg-white transition hover:-translate-y-[2px] hover:${
 ...
 <p className="text-[12.5px] font-bold text-[#0D0E12]">{template.name}</p>
 <span
 ...
 <p className="mb-[10px] text-[11px] leading-[1.5] text-[#6B6860]">
 ...
 {tags.map((tag) => (
 <span
 className="rounded-full bg-[#ECEAE6] px-[8px] py-[2px] text-[10px] font-semibold text-[#6B6860]"
 key={tag}
 >
 {tag}
 </span>
 ...
 <div
 className={`scroll-mt-24 border border-[#E2DFD9] bg-white p-[18px_22px] ${editingId ? "ring-2 ring-[#cc27b0]/30" : ""}`}
 ref={editorRef}
 >
 ...
 <div className="border border-sp-blue/10 p-4" key={step.id ?? `new-${index}`}>
 ...
 <select
 className="h-10 border border-sp-blue/15 bg-white px-3 text-sm"
 onChange={(e) => {
 const next = [...steps];
 ...
 <select
 className="mt-2 h-10 w-full border border-sp-blue/15 bg-white px-3 text-sm"
 onChange={(e) => {
 const next = [...steps];
 ...
 <div className="border-b border-[#ECEAE6] p-[13px_18px]">
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Active assignments</p>
 <p className="text-[11px] text-[#6B6860]">Ramp progress across assigned SEs</p>
 </div>
 {plans.length === 0 ? (
 <p className="px-[18px] py-8 text-center text-sm text-[#A09D98]">No plan assignments yet.</p>
 ) : (
 plans.map((plan) => {
 ...
 <p className="text-[12px] font-semibold text-[#3D3C38]">{se?.fullName ?? "Assigned SE"}</p>
 ...
 <span className="text-[10px] text-[#6B6860]">{progress}</span>
 ...
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Assign plan to SE</p>
 ...
 <div className="border border-[#E2DFD9] bg-white p-[18px_22px]">
 <p className="text-[12.5px] font-bold text-[#0D0E12]">All templates</p>
 <p className="mb-[14px] mt-[2px] text-[11px] text-[#6B6860]">
