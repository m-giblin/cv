...
const ACTIVITY_VISUAL: Record<string, { icon: typeof Bot; bg: string; color: string }> = {
 ...
 pitch_submitted: { icon: Video, bg: "#fce7f3", color: "#be185d" },
 flight_check_completed: { icon: TrendingUp, bg: "#e0f2fe", color: "#0369a1" },
};
...
export function SeWorkspaceNorthstar({
 ...
 "Pitch Studio": {
 badge: "Not started",
 badgeClass: "bg-[#ECEAE6] text-[#6B6860]",
 ...
 {/* Dark hero banner */}
 <div className="relative overflow-hidden bg-gradient-to-br from-[#001228] via-[#001f4a] to-[#002468] p-5 lg:p-6">
 ...
 {nextStep ? (
 <div className="mt-3.5 flex max-w-[660px] flex-col gap-3 border border-white/[0.12] bg-white/[0.07] p-3 sm:flex-row sm:items-center sm:justify-between">
 ...
 <div className="mt-3.5 max-w-[660px] border border-white/[0.12] bg-white/[0.07] p-3">
 <p className="text-[9.5px] font-bold uppercase tracking-[0.07em] text-white/40">
 Field readiness
 </p>
 ...
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Ramp progress</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">
 {plan?.progress ?? 0}%
 </p>
 <div className="mt-2 h-[3px] overflow-hidden rounded-full bg-[#e8f2fc]">
 <AnimatedProgressFill percent={plan?.progress ?? 0} />
 </div>
 <p className="mt-1.5 text-[10px] text-[#A09D98]">
 {validatedCount} of {sortedSteps.length || "—"} steps validated
 </p>
 </div>
 <div className={cn(CARD_SHELL, "border-l-[3px] border-l-[#10b981] p-4")}>
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Avg sim score</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">
 {simAvg ?? "—"}
 </p>
 ...
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Cert gates</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">
 {approvedCertCount}
 <span className="text-base font-medium text-[#A09D98]"> / 5</span>
 </p>
 ...
 <p className="mb-2 text-[10px] font-semibold text-[#6B6860]">Coaching cards</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">
 {cards.length}
 </p>
 <p className="mt-2 text-[10px] text-[#A09D98]">
 {pendingReviewCards > 0
 ? `${pendingReviewCards} awaiting manager review`
 : cards.length > 0
 ? "On record"
 : "None yet"}
 </p>
 ...
 <div>
 <p className="font-display text-[14px] font-bold text-[#0D0E12]">Practice toolkit</p>
 <p className="text-[11px] text-[#6B6860]">Your full training suite — pick up where you left off</p>
 </div>
 {practiceStreak >= 2 ? (
 ...
 return (
 <div className={cn(CARD_SHELL, "overflow-hidden transition hover:border-[#0071CE]/30")} key={tool.title}>
 <div className="h-[3px]" style={{ background: tool.topGradient }} />
 <div className="px-3.5 pb-3 pt-3.5">
 <div className="mb-2.5 flex items-center justify-between gap-2">
 <div className={cn("flex h-8 w-8 items-center justify-center ", tool.iconBg)}>
 <Icon className="h-4 w-4" style={{ color: tool.accent }} />
 </div>
 {meta.badge ? (
 <span className={cn("font-mono text-[8px] uppercase tracking-[0.08em] px-[7px] py-0.5 text-[9px] font-bold", meta.badgeClass)}>
 {meta.badge}
 </span>
 ) : null}
 </div>
 <p className="text-[12.5px] font-bold text-[#0D0E12]">{tool.title}</p>
 <p className="mt-0.5 text-[10.5px] leading-snug text-[#6B6860]">{tool.description}</p>
 <p className="mb-2.5 mt-2 text-[10px] text-[#A09D98]">{meta.activity}</p>
 ...
 <p className="text-[9.5px] font-bold uppercase tracking-[0.07em] text-[#6B6860]">
 Pre-call workflow
 </p>
 ...
 <div className={cn("flex shrink-0 items-center gap-2 px-3 py-2", step.bg)}>
 <div
 className={cn(
 ...
 <div>
 <p className="text-xs font-bold text-[#0D0E12]">Latest coaching card</p>
 ...
 <span className="truncate text-[10px] text-[#6B6860]">{bar.label}</span>
 <span className="text-[10px] font-semibold text-[#0D0E12]">{bar.score}</span>
 </div>
 ...
 <div className="border-l-[3px] border-l-[#0071ce] bg-[#F9F8F6] px-3 py-2.5">
 <p className="text-[11px] italic leading-relaxed text-[#3D3C38]">
 &ldquo;{latestCard.managerComments ?? latestCard.managerSummary}&rdquo;
 </p>
 {manager ? (
 <p className="mt-1 text-[10px] text-[#A09D98]">Manager: {manager.fullName}</p>
 ) : null}
 </div>
 ) : null}
 ...
 <p className="text-sm text-[#A09D98]">Complete a simulation to receive your first coaching card.</p>
 )}
 </div>
 </div>
 ...
 <p className="text-xs font-bold text-[#0D0E12]">Competency snapshot</p>
 <p className="text-[10px] text-[#A09D98]">Based on last 7 coaching cards</p>
 </div>
 ...
 <span className="text-[11.5px] font-semibold text-[#3D3C38]">{row.name}</span>
 <span className="text-[10.5px] font-bold" style={{ color }}>
 {row.avg !== null ? `${row.avg}%` : "—"}
 </span>
 </div>
 <div className="h-1.5 overflow-hidden rounded-full bg-[#ECEAE6]">
 <div
 className="h-full rounded-full transition-all duration-700 ease-out"
 style={{ width: `${row.avg ?? 0}%`, background: color }}
 />
 </div>
 <p className="mt-0.5 text-[9.5px] text-[#A09D98]">
 ...
 <div className={cn(CARD_SHELL, "overflow-hidden")}>
 <div className="border-b border-[#ECEAE6] px-4 py-3">
 <p className="text-xs font-bold text-[#0D0E12]">Recent activity</p>
 </div>
 <div className="py-1">
 {myActivity.length === 0 ? (
 <p className="px-4 py-6 text-center text-sm text-[#A09D98]">No recent activity yet.</p>
 ) : (
 myActivity.map((item) => {
 ...
 <div className="min-w-0 flex-1">
 <p className="text-[11px] font-semibold leading-snug text-[#3D3C38]">{item.title}</p>
 <p className="text-[10px] text-[#A09D98]">
 {person?.fullName ?? "You"} ·{" "}
