...
function statusPill(status: string) {
  ...
  return (
    <span
      className={`shrink-0 whitespace-nowrap rounded-full px-[9px] py-[3px] text-[9.5px] font-bold ${styles[status] ?? styles.not_started}`}
    >
      {label}
    </span>
  );
...
export function CertificationGateCard({
  ...
  const titleColor = isLocked ? "#475569" : "#0a1628";
  ...
  return (
    <div
      className={`rounded-xl bg-white p-[16px_18px] ${isLocked ? "border-[1.5px] border-[#e2eaf5] opacity-[0.68]" : ""}`}
      style={
        isLocked
          ? undefined
          : isOpen
            ? { border: "2px solid rgba(0,113,206,0.18)", boxShadow: "0 2px 12px rgba(0,113,206,0.07)" }
            : cert.status === "submitted" && isManager
              ? { border: "2px solid rgba(245,158,11,0.25)", boxShadow: "0 2px 12px rgba(245,158,11,0.08)" }
              : { border: "1.5px solid #e2eaf5" }
      }
    >
      <div className="mb-2 flex items-start justify-between">
        <div>
          <p className="mb-[3px] text-[9.5px] font-bold uppercase tracking-[0.06em] text-[#64748b]">
      ...
      <p className={`mb-3 text-[11.5px] leading-[1.6] ${isLocked ? "text-[#94a3b8]" : "text-[#64748b]"}`}>
        {details?.description ?? "Field readiness gate — evidence + manager sign-off."}
      </p>

      {isLocked && prerequisiteLabel ? (
        <p className="flex items-center gap-[5px] text-[10.5px] text-[#94a3b8]">
          <svg width="12" height="12" viewBox="0 0 16 16" fill="none" stroke="#94a3b8" strokeWidth="1.5" strokeLinecap="round">
      ...
      {cert.evidence_text ? (
        <div className="mb-3 rounded-[9px] border border-[#e2eaf5] bg-[#f8fafd] p-3">
          <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-[#64748b]">Evidence</p>
      ...
      {cert.manager_notes && cert.status !== "not_started" ? (
        <div className="mb-3 rounded-[9px] border border-[#e2eaf5] p-3">
          <p className="text-[10px] font-bold uppercase tracking-[0.06em] text-[#64748b]">Manager notes</p>
          <p className="mt-1 text-[12px] leading-[1.55] text-[#374151]">{cert.manager_notes}</p>
          {cert.approved_at ? (
            <p className="mt-1 text-[10px] text-[#94a3b8]">Reviewed {new Date(cert.approved_at).toLocaleDateString()}</p>
          ) : null}
        </div>
      ) : null}

      {isOpen ? (
        <div className="mb-2 rounded-[9px] border border-[#e2eaf5] bg-[#f8fafd] p-3">
          <p className="mb-2 text-[11px] text-[#64748b]">{details?.evidenceHint}</p>
          <label className="mb-[5px] block text-[10.5px] font-semibold text-[#475569]">Evidence description *</label>
          <textarea
            ...
            value={evidenceText}
          />
          <label className="mb-[5px] mt-2 block text-[10.5px] font-semibold text-[#475569]">
          ...
          <input
            ...
            className="mt-2 block w-full text-[11px] text-[#64748b]"
      ...
      {canReview ? (
        <div className="space-y-3 rounded-[9px] border border-[#fde68a] bg-[#fef9ec] p-3">
          <textarea
            className={SP_TEXTAREA_CLS}
...
export function CertificationProgressHero({
  ...
  return (
    <div className="mb-[18px] flex items-center gap-6 rounded-xl border border-[#e2eaf5] bg-white p-[18px_22px] shadow-[0_1px_4px_rgba(0,20,58,0.04)]">
      <ScoreRing
      ...
      <div className="flex-1">
        <p className="text-[13.5px] font-bold text-[#0a1628]">
