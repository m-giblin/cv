...
const OUTLINE_BTN =
  "inline-flex items-center bg-white text-[#334155] border border-[#e2eaf5] text-[11.5px] font-semibold px-[13px] py-[6px] rounded-lg hover:bg-[#f8fafd]";
...
function LearnCard({ card }: { card: LearnCardConfig }) {
  return (
    <div className="overflow-hidden rounded-xl border border-[#e2eaf5] bg-white shadow-[0_1px_4px_rgba(0,20,58,0.04)] transition hover:-translate-y-0.5 hover:shadow-[0_8px_28px_rgba(0,20,58,0.1)]">
      ...
      <div className="p-[16px_18px]">
        <div className="mb-3 flex items-center gap-[11px]">
          ...
          <div>
            ...
            <p className="text-[10.5px] text-[#64748b]">{card.sub}</p>
          </div>
        </div>
        ...
        <ul className="mb-[13px] space-y-[5px]">
          {card.bullets.map((bullet) => (
            <li key={bullet} className="flex gap-[7px]">
              ...
              <span className="text-[11px] text-[#475569]">{bullet}</span>
            </li>
          ))}
        </ul>
          ...
          {card.actions.map((action) => {
            ...
            if (action.variant === "solid") {
              return (
                <Link
                  key={action.label}
                  className="inline-flex items-center rounded-lg px-[13px] py-[6px] text-[11.5px] font-semibold text-white"
                  href={action.href}
...
export function LearnPageNorthstar() {
  return (
    <div className="space-y-[14px]">
      ...
      <div
        className="flex flex-col items-start justify-between gap-5 rounded-[13px] p-[18px_22px] sm:flex-row sm:items-center"
      ...
      >
        ...
        <div className="flex shrink-0 flex-wrap gap-2">
          <Link
            className="inline-flex items-center rounded-lg border px-3.5 py-2 text-[12px] font-semibold text-white"
            href="/challenges"
          ...
          <Link
            className="inline-flex items-center rounded-lg bg-[#0071ce] px-[18px] py-[9px] text-[12.5px] font-semibold text-white"
            href="/simulations"
          >
            Simulations →
          </Link>
