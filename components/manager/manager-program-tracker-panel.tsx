...
export function ManagerProgramTrackerPanel({
 ...
 <p className="text-[10px] font-semibold text-[#6B6860]">Active ramp plans</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">{activePlans.length}</p>
 </div>
 <div className={`${CARD} border-l-[3px] border-l-[#10b981] p-[14px_16px]`}>
 <p className="text-[10px] font-semibold text-[#6B6860]">Completed</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">{completeCount}</p>
 </div>
 <div className={`${CARD} border-l-[3px] border-l-[#cc27b0] p-[14px_16px]`}>
 <p className="text-[10px] font-semibold text-[#6B6860]">Team avg progress</p>
 <p className="font-display text-[26px] font-extrabold leading-none text-[#0D0E12]">{avgProgress}%</p>
 </div>
 </div>

 <div className={CARD}>
 <div className="border-b border-[#ECEAE6] p-[16px_18px]">
 <p className="text-[12.5px] font-bold text-[#0D0E12]">Ramp stage completion</p>
 <p className="text-[10.5px] text-[#A09D98]">Progress by onboarding week template across your team</p>
 </div>
 ...
 <span className="font-semibold text-[#0D0E12]">
 {row.shortLabel} — {row.stageName.replace(/^Week\s+\d+[–-]\d+\s*—\s*/i, "")}
 </span>
 <span className="shrink-0 text-[#6B6860]">
 ...
 <div className={CARD}>
 <div className="border-b border-[#ECEAE6] p-[16px_18px]">
 <p className="text-[12.5px] font-bold text-[#0D0E12]">SE plan status</p>
 </div>
 ...
 const statusBg = status === "Complete" ? "#dcfce7" : status === "In progress" ? "#dbeafe" : "#ECEAE6";
 const statusColor = status === "Complete" ? "#15803d" : status === "In progress" ? "#1d4ed8" : "#6B6860";
 return (
 ...
 <p className="truncate font-semibold text-[#0D0E12]">{person.fullName}</p>
 <p className="truncate text-[10px] text-[#A09D98]">{plan?.name ?? "No plan assigned"}</p>
 </div>
 </div>
