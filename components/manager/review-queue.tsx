...
export function ManagerReviewQueue({ items }: { items: ReviewItem[] }) {
  ...
  return (
    <div className="space-y-3">
      {items.map((item) => (
        <div className="rounded-2xl border border-amber-200/80 bg-white p-4" key={`${item.kind}-${item.id}`}>
          ...
          {item.kind === "coaching" && activeId === item.id ? (
            <div className="mt-4 space-y-4 border-t border-sp-blue/10 pt-4">
              ...
              <p className="rounded-2xl bg-sp-blue-soft/40 p-4 text-sm leading-6 text-sp-navy-muted">
                {item.managerSummary}
              </p>
              {item.transcript ? (
                <div>
                  ...
                  {expandedTranscript === item.id ? (
                    <pre className="mt-2 max-h-64 overflow-y-auto whitespace-pre-wrap rounded-2xl bg-slate-50 p-4 text-xs leading-5 text-slate-600">
                      {item.transcript}
                    </pre>
                  ) : null}
                </div>
