...
const RESOURCE_ACCENTS: Record<string, { accent: string; tagBg: string; tagColor: string }> = {
 ...
 default: { accent: "linear-gradient(90deg,#6B6860,#A09D98)", tagBg: "#ECEAE6", tagColor: "#6B6860" },
};
...
export function ContentLibraryBrowser({
 ...
 <div className="flex flex-wrap items-center gap-2 border border-[rgba(0,113,206,0.15)] bg-[#f0f7ff] px-3.5 py-2.5 text-[12px]">
 <Rocket className="h-4 w-4 text-[#0071ce]" />
 <span className="font-semibold text-[#0D0E12]">Your release training:</span>
 ...
 <div className="flex items-center gap-[18px] bg-gradient-to-br from-[#001228] to-[#002468] p-[16px_20px]">
 ...
 <a
 className="inline-flex items-center border border-white/15 bg-white/10 px-[13px] py-[6px] text-[11.5px] font-semibold text-white hover:bg-white/15"
 ...
 <div className="flex h-16 w-[100px] shrink-0 items-center justify-center border border-[rgba(0,113,206,0.4)] bg-[rgba(0,113,206,0.3)]">
 <SailPointLogoMark variant="flat-white" />
 </div>
 </div>

 <div className="flex flex-wrap items-center gap-[10px]">
 <div className="flex max-w-[320px] flex-1 items-center gap-2 border-[1.5px] border-[#E2DFD9] bg-white px-[13px] py-2">
 <svg width="14" height="14" viewBox="0 0 14 14" fill="none" stroke="#A09D98" strokeWidth="1.3" strokeLinecap="round">
 <circle cx="6" cy="6" r="4.5" />
 <line x1="9.5" x2="12.5" y1="9.5" y2="12.5" />
 </svg>
 <input
 className="flex-1 bg-transparent text-[12px] text-[#3D3C38] outline-none placeholder:text-[#A09D98]"
 ...
 {FILTERS.map((filter) => (
 <button
 className={`font-mono text-[8px] uppercase tracking-[0.08em] px-[14px] py-[6px] text-[11.5px] font-semibold transition ${
 activeFilter === filter
 ? "border border-[rgba(0,113,206,0.25)] bg-[#e8f2fc] text-[#0057a8]"
 : "border border-[#E2DFD9] bg-white text-[#3D3C38] hover:bg-[#F9F8F6]"
 }`}
 key={filter}
 ...
 <p className="py-8 text-center text-[12.5px] text-[#6B6860]">No resources match your search.</p>
 ) : (
 ...
 return (
 <div
 className="overflow-hidden border border-[#E2DFD9] bg-white transition hover:border-[#0071CE]/30"
 key={asset.id}
 >
 ...
 <span className="text-[9.5px] text-[#A09D98]">{asset.contentType ?? "PDF"}</span>
 </div>
 <p className="mb-1 text-[13px] font-bold leading-[1.4] text-[#0D0E12]">{asset.title}</p>
 <p className="mb-2.5 text-[11px] leading-[1.5] text-[#6B6860]">
 ...
 <button
 className="ml-auto inline-flex items-center text-[#6B6860] hover:text-[#0071ce]"
