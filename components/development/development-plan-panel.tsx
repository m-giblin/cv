...
const CARD_SHELL = "border border-[#E2DFD9] bg-white";
...
export function DevelopmentPlanPanel({
 ...
 <h2 className="flex items-center gap-2 text-lg font-semibold text-[#0D0E12]">
 ...
 <div className="border border-sp-blue/10 p-4" key={index}>
 <Input
 className="mb-2"
 onChange={(event) => {
 const next = [...goals];
 ...
 />
 <select
 className="mb-2 h-10 w-full border border-sp-blue/15 bg-white px-3 text-sm"
 onChange={(event) => {
 const next = [...goals];
 ...
 <h2 className="text-lg font-semibold text-[#0D0E12]">No development plan for {year}</h2>
 <p className="text-sm text-[#6B6860]">
 {viewerRole === "se"
 ...
 style={
 isActive
 ? { border: "1.5px solid rgba(0,113,206,0.2)", }
 : { border: "1.5px solid #E2DFD9" }
 }
 >
 <div className="flex items-center justify-between border-b border-[#ECEAE6] p-[14px_18px_12px]">
 <div className="flex items-center gap-2.5">
 <div className="flex h-8 w-8 shrink-0 items-center justify-center bg-[#e8f2fc]">
 <Target className="h-4 w-4 text-[#0071ce]" />
 </div>
 <div>
 <div className="flex flex-wrap items-center gap-1.5">
 <p className="text-[13px] font-bold text-[#0D0E12]">{goal.title}</p>
 <NorthstarGoalStatusBadge status={goal.overallStatus} />
 </div>
 <p className="text-[10.5px] text-[#6B6860]">
 {competency
 ...
 <div className="border-t border-[#ECEAE6] p-[10px_18px]">
 <ReviewEditor
...
function ReviewEditor({
 ...
 <>
 {review.seEvidence ? (
 <p className="bg-sp-blue-soft/30 p-3 text-sm text-sp-navy">{review.seEvidence}</p>
 ) : null}
 <Textarea
